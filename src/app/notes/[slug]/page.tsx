...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = getArticle("notes", slug);
  if (!article) return {};
  return {
...
export default async function NotesArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = getArticle("notes", slug);
  if (!article) notFound();

  const nextArticle = getNextArticle("notes", slug);
  ...
  return (
    <ArticleLayout frontmatter={article.frontmatter} category="notes" nextArticle={nextArticle}>
