...
description: "The AI-assisted debugging flow. The key is minimal reproduction — everything else follows from that."
category: workflows
order: 3
...
For frontend bugs, don't debug from a pasted error string alone. Open the app inside Cursor's browser, reproduce the bug there, and look at the actual runtime signals: console logs, failed network requests, warnings, visible DOM state, and whether the UI breaks before or after an interaction.

That matters because a lot of bugs are only obvious in the browser: hydration mismatches, broken form submissions, race conditions after navigation, event handlers that never fire, or client-side state that diverges from what the server rendered. A terminal stack trace often misses the part you actually need.
...
Browser context helps here too. A model that can see the runtime evidence is less likely to invent a cause that sounds plausible but doesn't match what's actually happening in the page.
