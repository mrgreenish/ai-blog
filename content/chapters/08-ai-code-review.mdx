...
relatedSlugs: [diff-review-loops, agent-guardrails, bug-to-fix]
chapter: 8
...
The workflow: make your changes and open the PR. BugBot can review automatically on each PR update; when you need a manual pass, comment `cursor review` or `bugbot run` on the PR. Fix the useful findings, dismiss the noise, then request human review. By the time a human sees the code, much of the mechanical first pass is already resolved.
...
Both review the PR diff with repository-specific guidance. Codex follows applicable `AGENTS.md` instructions, while BugBot follows its repository rules and applicable `.cursor/BUGBOT.md` files. Either can use that context to catch inconsistencies with nearby patterns; the practical difference is which review workflow and configuration your team already uses.
