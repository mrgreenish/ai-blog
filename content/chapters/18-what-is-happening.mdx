...
Cursor's iOS app is now [available in public beta](https://cursor.com/blog/ios-mobile-app) for paid plans. The pitch is not "write Swift on a phone"; it's "keep the agent queue moving when you're away from the laptop." You can start cloud agents from iPhone, point them at a repo, review plans and diffs, comment back into the run, and pick up the same work in desktop Cursor later. Cursor is also discounting Composer 2.5 runs in the mobile app by 75% through July 5, which is the right incentive: use the phone for low-friction delegation and follow-up, not for expensive frontier-model deep work.
...
Then the launch became a governance story. After U.S. government restrictions disrupted access to Fable 5 and Mythos 5, Commerce [allowed a limited return](https://www.axios.com/2026/06/27/commerce-anthropic-mythos-restrictions-lift) for Mythos 5 on June 26, mainly for approved cyber-defense and critical-infrastructure use. Public Fable 5 access is still the unresolved piece: reporting says Anthropic is working to broaden access again, but the model is no longer just a product rollout — it's part of an emerging, ad hoc approval process for frontier systems with offensive-security implications.

Practical read: Fable 5 matters less as "Anthropic's new best model" than as the first visible collision between broadly useful frontier capability and government-level release control. If it comes back broadly, the everyday takeaway is simple: use it for hard reasoning and agentic coding where Opus 4.8 is not enough, keep cheaper models on routine work, and expect some security-adjacent prompts to fall through to safer routes. If access stays constrained, the takeaway is bigger: the next model tier may ship less like SaaS and more like controlled infrastructure.
