...
## CI failures are becoming self-healing

The review agent catches what shouldn't merge. The CI Autofix automation catches what breaks after it does.

[Cursor's CI Autofix](https://cursor.com/marketplace/automations/ci-autofix) is a first-party automation in the Marketplace — the one-click version of a pattern you can also wire up manually via the [CLI cookbook](https://cursor.com/docs/cli/cookbook/fix-ci). Install it on a repo and a cloud agent wakes up every time your CI pipeline fails. It identifies the PR associated with the failing workflow run, traces the failure through the codebase, makes minimal targeted edits consistent with the repo's style, and then — this is the careful part — creates a separate fix branch rather than committing directly to the PR branch. It posts a single PR comment explaining what it did, with an inline compare link so you can one-click open a PR from the fix.

The trigger is a `workflow_run` event watching your test workflow for a `completed` + `failure` conclusion. The Cursor agent runs headless in GitHub Actions, reads the logs, and does the investigation in the same environment where the failure happened.

The design decision matters: CI Autofix proposes via a compare link rather than force-pushing to your branch. Bugbot Autofix commits directly to the PR branch — it's reviewing your own code and has standing to do that. CI Autofix creates a parallel fix branch because the failure might involve anyone's code, and the fix deserves its own history. You stay in control of whether the fix lands.

The Sentry integration shows the same pattern working across external signals. An Issue updated event triggers the automation, the agent reads the full stack trace and breadcrumbs via the Sentry MCP, traces the regression vector through `git log`, and opens a fix PR. Memories is wired in so the agent doesn't open duplicate PRs for the same issue. The [full cookbook](https://sentry.io/cookbook/regressed-issue-to-pr-cursor/) covers the setup in detail — including the fixability heuristic: the agent skips anything requiring architectural changes or touching more than five files.

Practical read: the interrupt is the expensive part. A CI failure that used to mean "get a Slack notification, context-switch, diagnose it, fix it, push, wait for CI again" can now land as "click this compare link and review the agent's fix." The human review step stays; the context-switch mostly disappears. Watch the first ten runs to calibrate fix quality before you trust it in production.
...
- **[Cursor CI Autofix](https://cursor.com/marketplace/automations/ci-autofix)**
  First-party Marketplace automation that wakes on CI `workflow_run` failures, traces root cause through git history, and posts a PR comment with a compare link to a separate fix branch — human review stays in the loop, context-switch disappears. One-click install; the underlying mechanism is the [CLI cookbook](https://cursor.com/docs/cli/cookbook/fix-ci) if you want to wire it manually.

- **[Tweet: cursor_ai on CI Autofix](https://x.com/cursor_ai/status/2051739625958584659)**
  Official announcement of the CI Autofix Marketplace automation.

- **[Cursor Automations](https://cursor.com/blog/automations)**
  The launch post for always-on cloud agents: trigger types, cloud sandbox mechanics, Memories, and how Bugbot Autofix was the first instantiation of the pattern. Worth reading before building custom automations.

- **[Sentry × Cursor Automations cookbook](https://sentry.io/cookbook/regressed-issue-to-pr-cursor/)**
  Step-by-step setup for auto-fixing Sentry regressions with Cursor Automations. Shows the full pattern: trigger config, Sentry MCP scopes, git regression tracing, fixability assessment, and PR creation. The most concrete reference for building event-driven fix automations against external signals.
