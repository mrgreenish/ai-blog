...
[Cursor's CI Autofix](https://cursor.com/marketplace/automations/ci-autofix) is a first-party automation in the Marketplace. Install it on a repo and a cloud agent wakes up every time your CI pipeline fails — reads the workflow logs, traces the failing change through git history, assesses whether the root cause lives in a handful of files and involves code-level changes, then either pushes a fix commit to the branch or posts a comment with a root cause hypothesis and the relevant commits to investigate.

The execution model is the same as Bugbot Autofix: isolated VM, full dev environment, type checker and linter pass before anything gets pushed. The trigger is different. Bugbot fires on PR creation and reviews the code ahead of time. CI Autofix fires on CI failure and responds to what broke. One is proactive, one is reactive — both run without you.
...
Practical read: the interrupt is the expensive part. A CI failure that used to mean "get a Slack notification, context-switch, diagnose it, fix it, push, wait for CI again" can now land as "review this commit the agent pushed." Install it, watch the first ten runs to calibrate fix quality for your codebase, then decide whether to let it auto-push or keep a review gate. The Sentry cookbook's rollout sequence — silent mode first, then comments, then a real PR — applies here too.
...
- **[Cursor CI Autofix](https://cursor.com/marketplace/automations/ci-autofix)**
  First-party Marketplace automation that wakes on CI failures, reads workflow logs, traces root cause through git history, and pushes fix commits or posts a diagnosis comment. One-click install. Pairs naturally with the security review automation — security review catches vulnerabilities, CI Autofix catches broken builds.
