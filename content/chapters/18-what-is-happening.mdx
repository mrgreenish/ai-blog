...
## Claude Opus 4.8 ships — more efficient and more persistent

Anthropic [released Claude Opus 4.8](https://www.anthropic.com/news/claude-opus-4-8), and it's available in Cursor now.

The Cursor read: on **CursorBench**, 4.8 works noticeably more efficiently than Opus 4.7. The other change is persistence — it pushes further on harder tasks before giving up or looping back for help.

Practical read: two concrete improvements that matter for the workloads Cursor users run most. The efficiency gain means less per-task cost for equivalent output. The persistence gain is the harder-to-benchmark one, but it's the difference between a run that stalls halfway through a complex refactor and one that finishes. Worth switching from 4.7 in Cursor to check if your longer tasks complete more cleanly.
