...
You spend the first twenty minutes of a session explaining your component to the model. The constraints. The edge cases. The things you explicitly decided not to build. By the end you have something that works — and a chat transcript that is not a durable, repo-visible source of truth for the next task.
...
The [Spec → PR workflow](/chapters/spec-to-pr) covers turning that context into a reviewed implementation. If the request begins in Jira, use the [Jira-to-Cursor workflow](/chapters/jira-to-cursor) to separate ticket-level outcomes from technical constraints that belong in the repository.

## Why spec files beat chat-only specs

Write a spec only in chat and it remains conversation context, not shared project documentation. A later conversation may be retrievable, but the next task, teammate, or tool will not reliably receive those decisions unless you attach them again. You either re-explain everything or the model makes assumptions.
...
Here's a real example — a compact spec for the current `Header` component in this codebase:
...
A fixed navigation bar rendered at the top of every page.

---

## Inputs

The component accepts no props. Its three navigation links are defined directly in the rendered JSX.
...
1. **Site link** — navigates to `/` and displays `AI Field Notes`
2. **Contents link** — navigates to `/#contents`
3. **What's Happening link** — navigates to `/chapters/what-is-happening`
...
All links use Next.js `Link`. Navigation styling is static, with hover color transitions and no active-route state.

---

## Constraints

- Remains fixed at `top-0` with `z-50`
- Max content width is `max-w-4xl` to match the page layout
- Uses existing foreground, background, and border tokens
- Adding or removing navigation items requires a code change
...
| Scenario | Behaviour |
|---|---|
| Narrow viewport | Links remain on one row; labels must stay concise |
| Long page content | Fixed header remains visible and content needs top spacing |
| JavaScript disabled | Next.js links still render as standard anchors |
