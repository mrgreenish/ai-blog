...
subtitle: "Why starting from atoms changes how AI writes code"
publishedAt: "2026-04-17"
updatedAt: "2026-07-22"
relatedSlugs: [design-to-storybook, agents-and-skills, spec-files]
chapter: 10
part: shipping-workflows
partNumber: 2
interactiveTools: [workflow-recipe]
illustrationPlaceholders:
  - "Atomic design diagram applied to AI coding: atoms to molecules to organisms. Annotate where AI helps most at each level."
  - "Before/after: AI generating a monolithic feature component vs building from typed atoms."
---
...
**Variants over conditionals.** Instead of `if (type === 'primary') ... else if (type === 'secondary')` scattered through a render function, define explicit variants. A helper such as `class-variance-authority` (`cva`) makes the available combinations visible at a glance and works well with Tailwind classes.
...
**Project rules** (`.cursor/rules/`) are persistent, version-controlled instructions that apply according to their configuration: always, by file pattern, when the model judges them relevant, or when you mention them manually. Mark a rule `alwaysApply: true` only when it must be present in every chat. They are the place to encode conventions that live in your head: "always use the `Button` component from `@/components/ui`, never create a new one," "spacing uses Tailwind's scale, never arbitrary values," "every new component gets a Storybook story."
...
**Project skills** — stored in locations such as `.cursor/skills/` or `.agents/skills/` — go further: reusable instruction sets for specific types of tasks. Creating a component, implementing a Figma design, setting up a new page. Where rules say "follow these conventions," skills say "here's the full workflow." Cursor discovers both locations; see the current [Skills documentation](https://cursor.com/docs/skills) for supported user-level and nested project paths.
