...
relatedSlugs: [agent-guardrails, spec-files, what-is-an-ai-harness]
chapter: 14
...
A strong public example of this pattern lives in the [official Next.js repository](https://github.com/vercel/next.js). The earlier `vercel-labs/next-skills` project moved there, and its workflows evolved:

**Next.js best practices** are now delivered through bundled documentation and generated agent rules. They cover:
...
**Upgrades** use the official codemod command:

```bash
npx @next/codemod@latest upgrade
```

**Cache Components** work is split into `next-cache-components-optimizer` and `next-cache-components-adoption`, separating optimization of an existing setup from adoption of the feature.

Install the current workflow skills with:
...
## A useful high-stakes skill: `ai-council`

The other pattern I keep reaching for is the opposite of a second pass — it's a *parallel* pass. Instead of asking one model a hard question and trusting the answer, ask three at once and let them peer-review each other.

That's what my [`ai-council`](https://github.com/mrgreenish/ai-council-plugin) plugin does. It spins up three subagents in parallel — GPT-5.5 as the adversarial analyst, Claude Opus 4.8 as the production-quality advocate, and Gemini 3.1 Pro as the breadth analyst — each returning the same structured schema (assumptions, risks ordered by severity, counterargument, confidence score). They then anonymously score each other's outputs, and the parent session synthesizes a final verdict informed by the peer review.

The point isn't ensemble averaging. It's that the three models have genuinely different instincts — the same personality differences from [Chapter 2](/chapters/model-personalities) — and surfacing all three at once catches blind spots that any single model would confidently miss.

When it's worth the spend:

- Architecture decisions where the wrong call costs weeks
- Code review on a high-stakes diff before you merge
- Ambiguous implementation choices with no clear winner
- "Best possible answer" requests where you'd otherwise re-ask three models manually

When to skip: simple lookups, quick bug fixes, formatting changes, anything obvious. Three frontier models running in parallel isn't free.

It ships as a Cursor plugin and also supports Claude Code. Current Cursor setup requires copying the project files or cloning the repository and adding the plugin from a local path; follow the [README](https://github.com/mrgreenish/ai-council-plugin) because the installation surface can change. Trigger it with `/ai-council` when you want the slower, more deliberate answer.
...
Skills compose with `CLAUDE.md`, `AGENTS.md`, and Cursor project rules under `.cursor/rules/`. Think of them as layers: the skill provides domain knowledge, the project config provides project-specific context, and the task prompt provides the specific request.
...
Claude Code stores project subagents as Markdown files under `.claude/agents/`. Each file defines the agent's instructions, tool access, model, and constraints in YAML frontmatter and a Markdown body. When the main agent encounters a task that matches the subagent's description, it can delegate.
