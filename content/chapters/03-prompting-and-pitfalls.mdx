---
title: "Prompting and Pitfalls"
subtitle: "The mindset shift that makes AI coding actually work"
publishedAt: "2026-04-17"
updatedAt: "2026-07-22"
chapter: 3
part: understanding-models
partNumber: 1
wonderQuestion: "Have you ever had the exact same task produce great results one day and terrible results the next, from the same model?"
interactiveTools: [scenario-lab]
illustrationPlaceholders:
  - "Before/after diagram: a vague prompt vs a structured prompt with context, constraints, and expected output format."
  - "Failure modes taxonomy: a visual map of the 5 most common AI coding mistakes."
---
...
For debugging, turn those habits into evidence: the [bug-report-to-fix workflow](/chapters/bug-to-fix) starts with a reproducible case and runtime signals instead of asking the model to guess from an error message.
...
Long, interdependent prompts can produce worse results than breaking the work into reviewable steps. More context, more assumptions, more output in one shot — errors compound. Independent, easily verified steps can still be batched when the shared context makes that cheaper and clearer.
...
For high-risk or behavior-changing code, read the relevant diff line by line before accepting it. For mechanical changes, generated files, or large repetitive edits, review the transformation and verify it with focused tests instead of pretending every line deserves equal attention. If you can't explain an important block, ask the model to explain it. If the explanation doesn't hold up, don't merge it.
