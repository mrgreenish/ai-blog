...
The [Spec → PR workflow](/workflows/spec-to-pr) covers writing a spec in chat before asking the model to plan and code. Works well for a single session. Problem is, the spec disappears when the conversation ends.

A `.spec.md` file solves that. Markdown file that lives next to the component it describes, gets committed to the repo, becomes the durable source of truth for what that component does — and what it doesn't do.
