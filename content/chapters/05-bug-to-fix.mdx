---
title: "AI Debugging: From Bug Report to Fix"
subtitle: "A repro-first workflow for diagnosing, fixing, and proving software bugs with AI"
...
The shortest reliable AI debugging workflow is: **reproduce the failure, capture runtime evidence, reduce it to the smallest failing case, explain the cause, make one narrow fix, and add a regression test**.
...
Use this minimal-reproduction template:
...
## Evidence
- Exact error or failed request:
- Console/server logs:
- First bad version or recent change:

## Minimal code
Only the files and data required to reproduce the failure.

## Constraints
Behavior and public APIs that must not change.
```

If those constraints should survive beyond this incident, capture them in a [spec file](/chapters/spec-files) rather than leaving them only in a chat.
...
**Step 7: Review the diff.** Confirm that the fix addresses the cause, not just the visible symptom. Use the [AI-generated code review checklist](/chapters/diff-review-loops) to inspect boundaries, test quality, security, and unrelated changes.
...
Prompting still matters, especially when evidence is incomplete. [Prompting and Pitfalls](/chapters/prompting-and-pitfalls) covers how to state constraints, ask for uncertainty, and keep a model from silently expanding the task.
