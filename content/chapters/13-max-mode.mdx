...
relatedSlugs: [reasoning-vs-fast, spec-files, model-personalities]
chapter: 13
...
Max Mode is a feature of Cursor's legacy request-based plans. It extends supported models beyond their standard context limits, which vary by model rather than sharing one universal default. Cursor currently documents million-token Max context for models including Claude Fable 5.1, Claude Sonnet 5, and Gemini 3.1 Pro. Check the [legacy-plan documentation](https://cursor.com/docs/account/pricing/request-based-legacy) for the current model matrix.
...
**When Auto handles it.** Cursor now offers Auto Cost, Auto Balance, and Auto Intelligence. Composer 2.5 draws from the Cursor Models pool with Grok 4.6 and Grok 4.5; Auto routing follows its own current pricing rules. If an Auto mode gives you a correct result, there is no reason to pay for broader context.
...
Cursor's allowances, usage pools, and overage rules vary by plan and can change. The stable part is the token math: a larger context means more input tokens billed at the selected model's applicable rate. Using the provider API rates checked on September 11, 2026:

- At raw API rates, a 400K-input / 60K-output session is about $0.268 on Luna, $2.68 on Terra, $5.00 on Sol, $12.50 on Astra, $7.00 on Fable 5.1, $2.10 on Kimi K3, $0.525 on Gemini 3.8 Flash, and $0.192 on DeepSeek-V4.1-Flash at peak rates before caching, tool fees, or platform charges. Gemini uses its introductory rate through December 31. OpenAI applies 2× input and 1.5× output pricing to the full request above 272K input tokens.
...
The calculator below lets you select these models and enter token counts directly. It includes the documented long-context rates; its optional 20% surcharge applies only to supported Max Mode requests on legacy Cursor plans. A model appearing in the API comparison does not guarantee that Cursor offers it.
