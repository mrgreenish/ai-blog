...
The model isn't broken. It's a very strong horse with nothing on it. Without a harness — rules about where it can step, what it has to check, and who it hands off to — it will just run.
...
Subagents, workflows, plan mode, browser agents, feedback loops — all the pieces you already use are parts of the same harness. We just didn't have a clean word for the whole system until recently.

The harness explains why workflow choices matter as much as model choice. [Reasoning vs fast AI models](/chapters/reasoning-vs-fast) covers where deeper reasoning pays off; the [Jira-to-Cursor workflow](/chapters/jira-to-cursor) shows the harness at work across ticket context, repository rules, permissions, tests, and review.
...
Three Markdown files. That's the MVP. To make future agents load them, reference them from a supported instruction surface such as `CLAUDE.md`, `AGENTS.md`, `.cursor/rules/`, or a relevant skill. Arbitrarily named Markdown files at the repository root are durable documentation, but they are not loaded automatically.
