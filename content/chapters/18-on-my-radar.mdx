...
Cursor's [My Machines](https://cursor.com/docs/cloud-agent/my-machines) takes self-hosted agents from an enterprise feature to an individual one. Instead of running in Cursor's managed VMs, your agent executes on hardware you control — your laptop, a devbox, a remote VM. One command: `agent worker start`.

The split is clean. Cursor handles inference and planning, sends tool calls to the worker process on your machine, results flow back over outbound HTTPS. The agent gets your local environment, your dependency caches, your internal network access. No repo syncing to a cloud sandbox, no missing internal packages, no secrets leaving your box.

Workers can be long-lived or single-use — handling sessions indefinitely or tearing down after each task. For teams, the Kubernetes operator and fleet management API handle scaling. For an individual developer, it's just a process running on a machine you already have.

"Cloud agent" used to mean "runs in a cloud VM." Now the cloud part is just the inference layer. Execution goes wherever makes sense — Cursor's sandboxes for isolation, your laptop for local deps, your company's cluster for compliance. The abstraction is pulling apart, and that's the interesting shift.
...
- **[Cursor My Machines](https://cursor.com/docs/cloud-agent/my-machines)**
  Connect your own laptop, devbox, or remote VM as a cloud agent worker. One command (`agent worker start`) turns any machine into an agent backend with local deps and network access.
