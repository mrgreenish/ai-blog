...
Most scalable way to apply guardrails: put them in `CLAUDE.md`, `AGENTS.md`, or scoped Cursor project rules under `.cursor/rules/`. Persistent context that applies to every task, not just the one you're working on.
...
The root `CLAUDE.md` provides project-wide instructions; nested `CLAUDE.md` files load when Claude works in their subdirectories. `SKILL.md` files are composable and domain-specific — loaded when relevant or when you invoke them explicitly.
...
See [Agents and Skills](/chapters/agents-and-skills) for how to write and structure skill files.
...
So: review every diff carefully, even when the agent followed all the guardrails. The [AI-generated code review checklist](/chapters/diff-review-loops) turns that review into explicit correctness, security, test, and human-judgment passes.
