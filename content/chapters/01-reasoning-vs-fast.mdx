...
illustrationPlaceholders:
  ...
  - "Cost comparison infographic: 10 common developer tasks mapped against cost per task on Opus, Sonnet, Haiku, GPT-5.4, and Gemini 3 Flash."
---
...
3. **Composer 2.5 for the test suite.** Gave it the implementation and said "full test coverage, including the race condition Opus flagged." It generated the tests, ran them, caught two failures from its own output, and fixed them autonomously.
4. **GPT-5.5 for review.** Asked it to review the final diff for edge cases and security issues. It spotted a missing rate limit on the new endpoint and a permissive CORS header I'd copied from another route.
