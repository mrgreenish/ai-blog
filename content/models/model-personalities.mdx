...
Benchmarks tell you what a model *can* do. Personality tells you what it *will* do when you don't spell out every detail. Two models with identical benchmark scores can produce wildly different results — not because one is smarter, but because they interpret ambiguity differently, take different amounts of initiative, default to different levels of caution.
...
Claude has opinions and isn't shy about sharing them. Pushes back on your approach, suggests alternatives, notices bugs you didn't ask about, restructures code to be "better" — even when you just wanted a simple change. With 4.6, Sonnet scores within 1.2 points of Opus on SWE-bench at one-fifth the cost, now with a 1M context window and automatic compaction for long sessions.
...
Composer-1.5 doesn't just edit files — it acts. Terminal commands, reading output, making more edits, looping until the task is done. Closest thing to an AI developer that can execute end-to-end.
...
Not limited to open files either. It navigates the project, finds relevant files, makes coordinated changes across many of them.
...
Match your prompt style to the model's instincts. Be explicit with Gemini — it won't infer intent. Set hard scope limits with Claude ("mention issues but don't fix them") or its helpfulness will expand your task. Give Composer-1.5 a clear success condition and let it run, but check in at breakpoints.
