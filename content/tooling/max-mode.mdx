...
description: "Max Mode can expand Cursor's standard context window up to 1M tokens — and on individual plans it bills at API rates + 20%. Here's when it earns that cost and when to leave it off."
...
By default, Cursor keeps you on the standard context window for the selected model. In practice that often means working in the 200K band — roughly 15,000 lines of code, which covers most single-feature work. Max Mode extends that to the model's maximum context window for models that support it, like GPT-5.4, Claude Sonnet 4.6, Claude Opus 4.6, and Gemini 3.1 Pro.

The catch: on individual plans it switches that session onto API-rate billing, plus a 20% upcharge. A few heavy Max Mode queries can chew through your included API usage fast.
...
**When Auto mode handles it.** Cursor's Auto mode selects a cost-efficient model automatically and draws from the separate Auto + Composer pool instead of your API pool. If Auto gives you a correct result, you have no reason to switch to Max.
...
**When cost matters more than completeness.** A few extended Max Mode sessions can exhaust a Pro plan's included $20 of API usage surprisingly fast. If you're on a budget or want predictable costs, stick to Auto for everyday work and use Max Mode surgically.
