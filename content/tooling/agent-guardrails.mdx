...
description: "Keeping AI agents on track. Scope constraints, confirmation rules, and CLAUDE.md patterns for reliable agent behavior."
category: tooling
order: 3
...
`CLAUDE.md` is always-on and project-wide. `SKILL.md` files are composable and domain-specific — the agent reads them when working in a specific area and applies the knowledge automatically.

This makes them a natural home for domain-scoped guardrails. A `SKILL.md` for your API layer might say "never return raw database errors to the client." A `SKILL.md` for your design system might say "never use arbitrary values — map everything to a semantic token." These constraints belong close to the domain knowledge they protect, not buried in a project-wide config file.

The layering works like this: `SKILL.md` provides domain guardrails, `CLAUDE.md` provides project-wide guardrails, and the task prompt provides the specific request. Each layer handles what it knows best.
